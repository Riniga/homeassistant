...
**Status: done.** Created `docs/architecture/decisions/README.md` with format, numbering, and when-to-use guidance. Decided **not** to add an ADR for the `docs` → `data` rename: applying the scaffold's own criteria, it's a routine, self-explanatory rename (`data/` holds exported inventory data, not documentation) already fully explained in `docs/architecture/overview.md`'s Major Components table — exactly the kind of change the scaffold says does *not* need one.

**Addendum (found during the Phase 6 completion review):** the direct-to-`main` git workflow decision (Assumptions, above; implemented in Phase 5) fits the scaffold's own criteria much better than the rename did — it changes a core convention, replaced a real documented alternative (branch + PR), and would be confusing to reverse-engineer from git history alone. Added as `docs/architecture/decisions/0001-direct-to-main-git-workflow.md`.
